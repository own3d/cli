...
function printAxiosError(e: any) { // deno-lint-ignore no-explicit-any
  if (e.response?.status === 422) {
    logError("Validation failed (422)");
...
export async function csUse(args: Args): Promise<number> {
  ...
  if (!id) {
    logError("Missing <codespace_id> argument. Usage: own3d cs:use <id>");
    return 1;
  }
  await setDefaultCodespaceId(id);
  success(`Default codespace set to ${id}`);
  return 0;
}
...
async function resolveId(args: Args): Promise<string> {
  ...
  if (envId) {
    info(`Using codespace from ENV OWN3D_CODESPACE_ID=${envId}`);
    return envId;
  }
  const stored = await getDefaultCodespaceId();
  if (stored) {
    info(`Using default codespace: ${stored} (override with --id or env OWN3D_CODESPACE_ID)`);
    return stored;
  }
  logError("No codespace id set. Provide --id, set OWN3D_CODESPACE_ID, or run 'own3d cs:use <id>'.");
  Deno.exit(1);
}

export async function csCreate(_args: Args): Promise<number> {
...
export async function csTree(args: Args): Promise<number> {
  ...
  try {
    const { data } = await axios.get(`${BASE_URL}/codespaces/${id}/fs/tree`, { headers: await getHeaders() });
    if (wantJson) {
...
export async function csLs(args: Args): Promise<number> {
  ...
  step(`Listing directory: ${pathArg}`);
  try {
    const { data } = await axios.get(`${BASE_URL}/codespaces/${id}/fs/ls`, { params: { path: pathArg }, headers: await getHeaders() });
    if (wantJson) { raw(JSON.stringify(data, null, 2)); return 0; }
    if (!data || data.type !== 'directory' || !Array.isArray(data.entries)) { logError('Unexpected API response shape (expected { path, type: "directory", entries: [] })'); return 1; }
    let entries: any[] = data.entries; // deno-lint-ignore no-explicit-any
    entries = sortEntries(entries);
    info("Contents:");
    if (!entries.length) { warn("(empty)"); return 0; }
    if (longOutput) {
      const nameWidth = Math.min(60, Math.max(...entries.map(e => (e.name || '').length), 4));
      raw(pad('TYPE', 12) + pad('NAME', nameWidth + 2) + 'ID');
      for (const entry of entries) {
        const rawType = entry.type || (entry.is_dir ? 'dir' : 'file');
        const type = (rawType === 'directory') ? 'dir' : rawType;
        const icon = (type === 'dir') ? '\ud83d\udcc1' : '\ud83d\udcc4';
        raw(pad(type, 12) + pad(`${icon} ${entry.name}`, nameWidth + 2) + (entry.id ?? '-'));
      }
    } else {
      for (const entry of entries) {
        const rawType = entry.type || (entry.is_dir ? 'dir' : 'file');
    ...
    return 0;
  } catch (e) { printAxiosError(e); return 1; }
}
...
export async function csRead(args: Args): Promise<number> {
  ...
  if (!path) { logError('Missing file path. Usage: own3d cs:read <path> [--id=<codespace>]'); return 1; }
  const id = await resolveId(args);
  try {
    const { data } = await axios.get(`${BASE_URL}/codespaces/${id}/fs/file`, { params: { path }, headers: await getHeaders() });
    raw(data);
    return 0;
  } catch (e) { printAxiosError(e); return 1; }
}
...
export async function csWrite(args: Args): Promise<number> {
  ...
  setLoggerQuiet(quiet);
  const path = typeof args.path === 'string' ? args.path : (typeof args._[0] === 'string' ? args._[0] as string : undefined);
  if (!path) { logError('Missing destination path. Usage: own3d cs:write <path> --file=localFile [--id=<codespace>]'); return 1; }
  const filePath = requireArg(args, 'file', 'Missing required --file local file path');
  const id = await resolveId(args);
  step(`Uploading file to: ${path}`);
  try {
    ...
    if (e instanceof Deno.errors.NotFound) { logError(`Local file not found: ${filePath}`); return 1; }
    printAxiosError(e); return 1;
  }
}
...
export async function csRm(args: Args): Promise<number> {
  ...
  setLoggerQuiet(quiet);
  const path = typeof args.path === 'string' ? args.path : (typeof args._[0] === 'string' ? args._[0] as string : undefined);
  if (!path) { logError('Missing file path. Usage: own3d cs:rm <path> [--id=<codespace>]'); return 1; }
  const id = await resolveId(args);
  step(`Deleting: ${path}`);
  try {
    await axios.delete(`${BASE_URL}/codespaces/${id}/fs/rm`, { headers: await getHeaders(), params: { path } });
    success('Deleted');
    return 0;
  } catch (e) { printAxiosError(e); return 1; }
}
...
export async function csReset(args: Args): Promise<number> {
  ...
  setLoggerQuiet(quiet);
  const id = await resolveId(args);
  warn('⚠️  Resetting filesystem to repository state (all changes lost)...');
  try {
    await axios.post(`${BASE_URL}/codespaces/${id}/fs/reset`, undefined, { headers: await getHeaders() });
    success('Filesystem reset');
    return 0;
  } catch (e) { printAxiosError(e); return 1; }
}
...
export async function csInfo(_args: Args): Promise<number> {
  ...
  if (resolved) {
    success(`Active codespace: ${resolved}`);
    info(`Source: ${source}`);
    warn(`Override order: --id > ENV OWN3D_CODESPACE_ID > stored default`);
    return 0;
  } else {
    error('No active codespace resolved.');
    info('Set one with: own3d cs:use <id>  OR  pass --id=<id>  OR  export OWN3D_CODESPACE_ID=<id>');
    return 1;
  }
}
